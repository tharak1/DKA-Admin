...
const MarksEntryCard:React.FC<MarksEntryCardProps> = ({performance,index,studentPerformances,setStudentPerformances}) => {
  ...
  return (
    ...
    <div className='grid grid-cols-4 max-sm:grid-cols-1 gap-2 mb-3 w-full'>
    ...
    <div className='grid grid-cols-3 max-sm:grid-cols-2 mb-3 dark:text-white'>
      {Object.entries(performance).map(([key, val]) => (
        key !== 'studentName' && key !== 'studentId' && key !== 'startDate' && key !== 'endDate' && key !== 'TotalClassesTaken' && key !== 'TotalClassesAttended' && key !== 'Grade' && key !== 'totalMarks' && key !== 'totalMarksObtained'  && (
          <div className="text-center flex justify-start items-center px-5 my-1" key={key}>
            <p className=" mr-3">{formatLabel(key)}</p>
            <input
              value={val as string}
              onChange={(e) => {
    ...
    <div className="text-center grid grid-cols-2 justify-start items-center my-1 ">
            ...
            <input
              ...
              type='text'
              className="text-xl w-full font-bold bg-gray-100 rounded-md border-2 border-gray-400 flex items-center justify-center dark:text-black"
            />
          </div>

          <div className="text-center justify-start items-center my-1 grid grid-cols-2">
            <p className=" mr-3">Classes Attended :</p>
            <input
