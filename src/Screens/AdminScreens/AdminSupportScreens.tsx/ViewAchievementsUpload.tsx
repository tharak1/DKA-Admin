...
import { fetchAchievementsUploads, selectAchievementsUploads } from '../../../redux/AchievementsUploadSlice';
import { useSelector } from 'react-redux';
import AchievementsUploadModal from '../AdminComponents/AchievementsUploadModal';
import { AchievementsUploadModel } from '../../../Models/CharityModel';
import AchievementsUploadCard from '../AdminComponents/AchievementsUploadCard';

const ViewAchievementsUploads: React.FC = () => {
  const [loading, setLoading] = useState<boolean>(false);
  const dispatch = useAppDispatch();

  useEffect(() => {
    setLoading(true);
    dispatch(fetchAchievementsUploads());
    setLoading(false);
  }, []);
      ...
      <div className='grid-span-3 w-full grid sm:grid-cols-3 max-sm:grid-cols-1 gap-3 mt-4 max-sm:mt-4 max-sm:p-3'>
        {
          loading ? <div className='w-full h-screen flex justify-center items-center'>
            ...
            achievementsUpload.map((obj: AchievementsUploadModel) => (
              <AchievementsUploadCard AchievementsUploadData={obj} key={obj.id} />
            ))
        }
      </div>
    </div>
...
export default ViewAchievementsUploads;
